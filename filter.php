...
<html lang="en">
  ...
  <body>
    ...
    $patient = $_POST['formPatient'];
    $clinician = $_POST['formClinician'];
    $room = $_POST['formRoom'];
    $dat = $_POST['formDate'];
    $timestamp = substr($dat,2);
	...
	<?php
	    ...
	    if ($patient == 'select' && $clinician == 'select' && $room == 'select' && $timestamp != ''){
	        $sql = "SELECT * FROM Session WHERE Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
       		$result = mysqli_query($db,$sql);
   	    }
	    
	    if ($patient == 'select' && $clinician == 'select' && $room != 'select' && $timestamp == ''){
	        $sql = "SELECT * FROM Session WHERE Room = $room ORDER BY Timestamp DESC";
       		$result = mysqli_query($db,$sql);
   	    }
	    
	    if ($patient == 'select' && $clinician == 'select' && $room != 'select' && $timestamp != ''){
	        $sql = "SELECT * FROM Session WHERE Title LIKE '%$timestamp%' AND Room = $room ORDER BY Timestamp DESC";
       		$result = mysqli_query($db,$sql);
   	    }
   	    
   	    if ($patient == 'select' && $clinician != 'select' && $room == 'select' && $timestamp == ''){
   	    	$sql = "SELECT * FROM Session WHERE Clinician = $clinician ORDER BY Timestamp DESC";
   	    	$result = mysqli_query($db,$sql);
       	    }
	    
	    if ($patient == 'select' && $clinician != 'select' && $room == 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Clinician = $clinician AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient == 'select' && $clinician != 'select' && $room != 'select' && $timestamp == ''){
	    	$sql = "SELECT * FROM Session WHERE Clinician = $clinician AND Room = $room ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient == 'select' && $clinician != 'select' && $room != 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Clinician = $clinician AND Room = $room AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician == 'select' && $room == 'select' && $timestamp == ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician == 'select' && $room == 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician == 'select' && $room != 'select' && $timestamp == ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Room = $room ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician == 'select' && $room != 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND AND Room = $room AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician != 'select' && $room == 'select' && $timestamp == ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Clinician = $clinician ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician != 'select' && $room == 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Clinician = $clinician AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician != 'select' && $room != 'select' && $timestamp == ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Clinician = $clinician AND Room = $room ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
	    
	    if ($patient != 'select' && $clinician != 'select' && $room != 'select' && $timestamp != ''){
	    	$sql = "SELECT * FROM Session WHERE Patient = $patient AND Clinician = $clinician AND Room = $room AND Title LIKE '%$timestamp%' ORDER BY Timestamp DESC";
	    	$result = mysqli_query($db,$sql);
	    }
